        ...
        merchant = {
            ...
            items = {
                ...
                {type = "egg", name = "Mega Brainrot Egg", cost = 375375, stockChance = 10, rarity = "UNSTABLE", pets = {
                    {Name = "Nooooo my hotspot", Chance = 30, Rarity = "Unbalance", Worth = 101101},
                    {Name = "La Esok Sekolah", Chance = 10, Rarity = "Unbalance", Worth = 109400},
...
function UpdateInventory()
    ...
    for i, egg in ipairs(PlayerData.Eggs) do
        ...
        hatchButton.MouseButton1Click:Connect(function()
            local eggInfo = EggData[egg.Name] or egg -- For merchant eggs, egg is the table
...
print("Pet System with Weather loaded successfully! Mobile Optimized: " .. tostring(isMobile))            {Name = "Golden Retriever", Chance = 7, Rarity = "Uncommon", Worth = 500},
            {Name = "Parrot", Chance = 1, Rarity = "Rare", Worth = 1200}
        }
    },
    ["Uncommon Egg"] = {
...
local WeatherData = {
    ["Clear"] = {
        ...
        visual = function()
            ...
            local particles = workspace:FindFirstChild("WeatherParticles")
            if particles then
                particles:Destroy()
            end
        end
...
-- Functions (moved after GUI creation)
local function GenerateStock()
    ShopStock = {}
    for eggName, eggInfo in pairs(EggData) do
        local stockAmount = 0
...
local function UpdateMerchantRestockTimer()
    local minutes = math.floor(TimeUntilMerchantRestock / 60)
    local seconds = TimeUntilMerchantRestock % 60
    merchantRestockLabel.Text = string.format("🔄 Restock: %d:%02d", minutes, seconds)
end

local function UpdateNextWeatherTimer()
        ...
        hatchButton.MouseButton1Click:Connect(function()
            ...
            if selectedPet then
                ...
                if CurrentWeather ~= "Clear" then
                    local mutations = WeatherData[CurrentWeather].mutations
                    for _, mut in ipairs(mutations) do
                        if math.random(1, 100) <= (100 / mut.chance) then
                            worth = math.floor(worth * mut.multiplier)
                            mutationText = " with " .. mut.name .. " mutation!"
                            break
                        end
