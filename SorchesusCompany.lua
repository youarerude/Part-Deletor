...
-- Fixed errors: concatenation nil with string, comparison nil <= number
-- Added close button to info gui
...
local function RollForMXGift(anomalyInstance)
    ...
    if data.MXWeapon and math.random() < data.MXWeapon.Chance then
        local item = {
            ...
            MinLevel = data.MXWeapon.MinLevel or 1,
            MaxLevel = data.MXWeapon.MaxLevel or 5,
            Type = "Weapon",
    ...
    if data.MXArmor and math.random() < data.MXArmor.Chance then
        local item = {
            ...
            MinLevel = data.MXArmor.MinLevel or 1,
            MaxLevel = data.MXArmor.MaxLevel or 5,
            Type = "Armor",
            Anomaly = anomalyInstance.Name,
            EquippedTo = nil
        }
...
function TriggerBreach(anomalyInstance, roomFrame)
    ...
    for _, anomaly in ipairs(GameData.OwnedAnomalies) do
        if anomaly.Name == "Prince of Fame" and not anomaly.IsBreached then
    ...
    spawn(function()
        ...
        while anomalyInstance.IsBreached do
            ...
            if anomalyInstance.Data.Special == "PrinceOfFame" and anomalyInstance.IsHelping then
                if #GameData.BreachedAnomalies <= 1 then  -- only itself
                    ...
                    if #targets > 0 then
                        ...
                        CreateNotification("The Fame Seeker attacked " .. target.Data.BreachForm.Name .. " for " .. damage, Color3.fromRGB(100, 200, 255))
                    ...
                    if anomaly.Name == "Prince of Fame" then
                        ...
                        if anomaly.Data.IsInanimate then
                            if anomaly.AssignedWorker then
                                ...
                                anomaly.CurrentMood = anomalyInstance.Data.BaseMood / 2
                                UpdateRoomDisplay(anomaly)
                            end
                        else
                            TriggerBreach(anomaly, anomaly.RoomFrame)
...
function UpdateCoreDisplay()
    ...
    CoreStatusLabel.Text = GameData.CosmicShardCoreHealth <= 0 and "STATUS: DESTROYED" or healthPercent < 0.3 and "STATUS: CRITICAL" or healthPercent < 0.6 and "STATUS: DAMAGED" or "STATUS: PROTECTED"
...
function CompanyDestroyed()
    ...
    CreateInstance("TextLabel", {
        ...
        Text = string.format("Final Crucible: %d\nAnomalies Contained: %d\nBreaches: %d", GameData.Crucible, #GameData.OwnedAnomalies, #GameData.BreachedAnomalies),
        Font = Enum.Font.Gotham,
        TextSize = 16,
        TextColor3 = Color3.fromRGB(200, 200, 200),
        TextWrapped = true,
