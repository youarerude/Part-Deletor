...
local function IsDisgraced(name)
    local e = EntityRegistry[name]
    return e and e.disgraced or false
end
...
local function RegisterEntity(name, symbolizes, desc, onEnable, onDisable)
    ...
    -- Disgrace toggle button (purple D, left of ON/OFF)
    local db = Instance.new("TextButton")
    db.Size             = UDim2.new(0,22,0,22)
    db.AnchorPoint      = Vector2.new(1,0.5)
    db.Position         = UDim2.new(1,-60,0.5,0)
    db.BackgroundColor3 = Color3.fromRGB(50,20,60)
    db.Text             = "D"
    db.Font             = Enum.Font.GothamBold
    db.TextSize         = 11
    db.TextColor3       = Color3.fromRGB(120,80,160)
    db.BorderSizePixel  = 0
    db.ZIndex           = 24
    db.Parent           = row
    Instance.new("UICorner", db).CornerRadius = UDim.new(0,5)

    local entry = { name=name, enabled=false, disgraced=false, onEnable=onEnable, onDisable=onDisable }
    ...
    db.MouseButton1Click:Connect(function()
        entry.disgraced = not entry.disgraced
        if entry.disgraced then
            db.BackgroundColor3 = Color3.fromRGB(100,30,140)
            db.TextColor3       = Color3.fromRGB(210,150,255)
        else
            db.BackgroundColor3 = Color3.fromRGB(50,20,60)
            db.TextColor3       = Color3.fromRGB(120,80,160)
        end
        -- If entity is running, restart it to apply disgraced state
        if entry.enabled and onDisable and onEnable then
            pcall(onDisable)
            task.wait(0.05)
            pcall(onEnable)
        end
...
local function BuildEyeBB(tp)
    ...
    local disgraced = IsDisgraced("Gaze")

    local bb = Instance.new("BillboardGui")
    bb.Name = "GazeEye"; bb.Size = UDim2.new(0,80,0,80)
    bb.StudsOffset = Vector3.new(0,0.5,0)
    bb.AlwaysOnTop = disgraced  -- see through walls when disgraced
    bb.Adornee = h; bb.Parent = h

    local ringCol  = disgraced and Color3.fromRGB(160,0,255) or Color3.fromRGB(255,40,40)
    local ringCol2 = disgraced and Color3.fromRGB(100,0,200) or Color3.fromRGB(255,140,0)
    local irisCol  = disgraced and Color3.fromRGB(40,0,60)   or Color3.fromRGB(15,0,0)

    local ring = Instance.new("Frame")
    ring.Size = UDim2.new(1,0,1,0); ring.BackgroundColor3 = ringCol
    ...
    iris.Position = UDim2.new(0.5,0,0.5,0); iris.BackgroundColor3 = irisCol
    ...
    pupil.BackgroundColor3 = disgraced and Color3.fromRGB(80,0,120) or Color3.fromRGB(0,0,0)
    ...
    TweenService:Create(ring, TweenInfo.new(0.9, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut, -1, true),
        {BackgroundColor3 = ringCol2}):Play()

    -- Disgraced: pupil shakes
    if disgraced then
        local shakeT = 0
        RunService.Heartbeat:Connect(function(dt)
            if not pupil or not pupil.Parent then return end
            shakeT = shakeT + dt
            local ox = (math.random()-0.5)*12
            local oy = (math.random()-0.5)*12
            pupil.Position = UDim2.new(0.5, ox, 0.5, oy)
        end)
    end
...
local function OnGazeEnable()
    ...
    Gaze.conn = RunService.Heartbeat:Connect(function()
        ...
        if Gaze.target then
            ...
            local looking
            if IsDisgraced("Gaze") then
                -- Disgraced: damage through walls always
                looking = true
                -- Weak camera force toward the eye
                local char2 = Gaze.target.Character
                if char2 then
                    local eyeHead = char2:FindFirstChild("Head")
                    if eyeHead then
                        local camCF    = Camera.CFrame
                        local toEye    = (eyeHead.Position - camCF.Position).Unit
                        local blended  = camCF.LookVector:Lerp(toEye, 0.04)
                        if blended.Magnitude > 0.001 then
                            Camera.CFrame = CFrame.new(camCF.Position, camCF.Position + blended)
                        end
                    end
                end
            else
                looking = GazeLookCheck(Gaze.target)
            end
...
local function BuildEludeModel()
    ...
    local disgraced = IsDisgraced("Elude")
    local DC = disgraced and Color3.fromRGB(80,0,100) or Color3.fromRGB(6,6,10)
    local hm = disgraced and 1.5 or 1.0
    ...
    local torso = mkP("Torso",    Vector3.new(1.8, 2*hm, 0.8),    0.12)
    mkP("Head",     Vector3.new(0.9,0.9,0.9),                      0.12)
    mkP("LeftArm",  Vector3.new(0.8, 1.8*hm, 0.8),                0.22)
    mkP("RightArm", Vector3.new(0.8, 1.8*hm, 0.8),                0.22)
    mkP("LeftLeg",  Vector3.new(0.8, 1.8*hm, 0.8),                0.22)
    mkP("RightLeg", Vector3.new(0.8, 1.8*hm, 0.8),                0.22)
    m.PrimaryPart = torso
    ...
    if hd then
        ...
        eyeBB.AlwaysOnTop = disgraced; eyeBB.Adornee = hd; eyeBB.Parent = hd
        local eyeC  = disgraced and Color3.fromRGB(0,0,0)    or Color3.fromRGB(0,220,200)
        local eyeC2 = disgraced and Color3.fromRGB(20,0,20)  or Color3.fromRGB(0,100,90)
        local function mkEye(ax)
            ...
            e.BackgroundColor3 = eyeC; e.BorderSizePixel=0; e.Parent=eyeBB
            ...
            TweenService:Create(e, TweenInfo.new(1.5, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut, -1, true),
                {BackgroundColor3=eyeC2}):Play()
        end
        mkEye(0); mkEye(1)
    end
    -- Disgraced: purple ESP rings always-on-top on torso
    if disgraced then
        local espBB = Instance.new("BillboardGui")
        espBB.Size = UDim2.new(0,130,0,130); espBB.StudsOffset = Vector3.new(0,0,0)
        espBB.AlwaysOnTop = true; espBB.Adornee = torso; espBB.Parent = torso
        local base = Instance.new("Frame")
        base.Size = UDim2.new(1,0,1,0); base.BackgroundTransparency=1
        base.BorderSizePixel=0; base.Parent=espBB
        for i,sz in ipairs({0.95,0.72,0.50}) do
            local r = Instance.new("Frame")
            r.Size=UDim2.new(sz,0,sz,0); r.AnchorPoint=Vector2.new(0.5,0.5)
            r.Position=UDim2.new(0.5,0,0.5,0); r.BackgroundTransparency=1
            r.BorderSizePixel=3; r.BorderColor3=Color3.fromRGB(160,0,220)
            r.ZIndex=2; r.Parent=base
            Instance.new("UICorner",r).CornerRadius=UDim.new(1,0)
            TweenService:Create(r, TweenInfo.new(0.8+i*0.2, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut, -1, true),
                {BorderColor3=Color3.fromRGB(220,120,255)}):Play()
        end
    end
...
local function EludeVisibleFromCam(pos)
    -- Disgraced: always visible regardless of walls
    if IsDisgraced("Elude") then return true end
    local camCF  = Camera.CFrame; local camPos = camCF.Position
    ...
    Norm.conn = RunService.Heartbeat:Connect(function(dt)
        ...
        if Norm.blockingActive and Norm.blockPart and Norm.blockPart.Parent then
            ...
            if Norm.blockTimer >= Norm.BLOCK_DUR then
                ...
                local distFromPlayer = (curPos - hrp.Position).Magnitude

                -- Speed: 100 if >15 studs away, else 13
                local chaseSpeed = distFromPlayer > 15 and 100 or 13
                -- Convert to spring-style: at high speed use direct move, at low speed use smooth spring
                local newPos
                if distFromPlayer > 15 then
                    ...
                    if mag > 0.1 then
                        local step = math.min(chaseSpeed * dt, mag)
                        newPos = curPos + dir.Unit * step
                    ...
                    -- Smooth spring at normal speed (13)
                    local SPRING = 3.5
                    local DAMP   = 0.55
                    Norm.blockVelX = Norm.blockVelX * (1 - DAMP) + diff.X * SPRING * dt
                    Norm.blockVelZ = Norm.blockVelZ * (1 - DAMP) + diff.Z * SPRING * dt
                    -- Clamp spring velocity to match speed cap
                    local velMag = math.sqrt(Norm.blockVelX^2 + Norm.blockVelZ^2)
                    if velMag > chaseSpeed * dt then
                        local scale = (chaseSpeed * dt) / velMag
                        Norm.blockVelX = Norm.blockVelX * scale
                        Norm.blockVelZ = Norm.blockVelZ * scale
                    end
                    newPos = Vector3.new(
...
-- ═══════════════════════════════════════════════════════════
--[[
    ▸ Every 3 minutes a NUKE EVENT triggers:
      PHASE 1 (15s warning):
        - A 50-stud red ESP circle appears on the ground under the player
        - Screen flashes red on a countdown: 3s → 2s → 1s → 0.5s → 0.1s intervals
          (each interval halves, so flashing accelerates)
        - After 15s a huge see-through NUKE model descends from 150 studs up
      PHASE 2 (nuke falls ~2s):
        - Nuke falls toward the center of the circle
      PHASE 3 (EXPLOSION):
        - Massive nuke explosion: shockwave ring, mushroom cloud, flash
        - Any player inside the 50-stud circle → -100% fate instantly
        - Extremely intense screen shake
        - Explosion parts auto-cleanup after 20s
--]]
...
local BlueSky = {
    ...
    dmgDealt    = false,
    -- Disgraced tracking
    disgLaunchCount = 0,   -- how many times launched this event
    MAX_LAUNCHES    = 3,
}

-- Blue Sky flash overlay
local BSFlash = Instance.new("Frame")
BSFlash.Name                    = "BSFlash"
BSFlash.Size                    = UDim2.new(1,0,1,0)
BSFlash.BackgroundColor3        = Color3.fromRGB(255,0,0)
BSFlash.BackgroundTransparency  = 1
BSFlash.ZIndex                  = 14
BSFlash.Parent                  = ScreenGui

-- Blue Sky warning text
local BSText = Instance.new("TextLabel")
BSText.Name                     = "BSText"
BSText.Size                     = UDim2.new(0,400,0,30)
BSText.AnchorPoint              = Vector2.new(0.5,0)
BSText.Position                 = UDim2.new(0.5,0,0,106)
BSText.BackgroundTransparency   = 1
BSText.Text                     = ""
BSText.Font                     = Enum.Font.GothamBold
BSText.TextSize                 = 20
BSText.TextColor3               = Color3.fromRGB(255,60,60)
BSText.TextTransparency         = 1
BSText.ZIndex                   = 14
BSText.Parent                   = ScreenGui

local function BSShowText(txt, dur)
    BSText.Text = txt
    TweenService:Create(BSText, TweenInfo.new(0.25), {TextTransparency=0}):Play()
    task.delay(dur or 2.5, function()
        TweenService:Create(BSText, TweenInfo.new(0.5), {TextTransparency=1}):Play()
    end)
end

-- Spawn the ground ESP circle (cylinder, see-through red)
local function SpawnNukeCircle(pos)
    local c = Instance.new("Part")
    c.Name        = "NukeCircle"
    c.Shape       = Enum.PartType.Cylinder
    c.Size        = Vector3.new(0.25, 100, 100)
    c.CFrame      = CFrame.new(pos + Vector3.new(0,0.14,0)) * CFrame.Angles(0,0,math.pi/2)
    c.Anchored    = true
    c.CanCollide  = false
    c.CastShadow  = false
...
-- Build the nuke missile model (see-through ESP style)
local function BuildNukeModel(spawnPos)
    local model = Instance.new("Model")
    model.Name  = "NukeMissile"

    local function mkP(nm, sz, pos, col, trans)
        local p = Instance.new("Part")
        p.Name        = nm
        p.Size        = sz
        p.CFrame      = CFrame.new(spawnPos + pos)
        p.Anchored    = true
        p.CanCollide  = false
        p.CastShadow  = false
    ...
    local body = mkP("Body",
        Vector3.new(3, 18, 3),
        Vector3.new(0,0,0),
        Color3.fromRGB(180,180,255), 0.55)
    -- Nose cone (wedge shape via special mesh)
    local nose = mkP("Nose",
        Vector3.new(3,5,3),
        Vector3.new(0,11.5,0),
        Color3.fromRGB(255,80,80), 0.5)
    local noseMesh = Instance.new("SpecialMesh")
    noseMesh.MeshType = Enum.MeshType.FileMesh
    noseMesh.MeshId   = "rbxasset://fonts/torso.mesh"
    noseMesh.Scale    = Vector3.new(1.5,1,1.5)
    ...
    for i = 0, 3 do
        ...
        local fin = mkP("Fin"..i,
            Vector3.new(0.4, 6, 3),
            Vector3.new(math.sin(angle)*2.2, -6, math.cos(angle)*2.2),
            Color3.fromRGB(140,140,255), 0.55)
    end
    -- Exhaust glow
    mkP("Exhaust",
        Vector3.new(2,1,2),
        Vector3.new(0,-9.5,0),
        Color3.fromRGB(255,160,0), 0.3)
...
-- Build explosion: flash sphere, shockwave ring, mushroom cloud column
local function SpawnNukeExplosion(pos)
    local parts = {}
    ...
    TweenService:Create(flash,
        ...
        {Size = Vector3.new(90,90,90), Transparency = 0.2}
    ):Play()
    task.delay(0.8, function()
        TweenService:Create(flash,
            TweenInfo.new(1.2),
            {Size = Vector3.new(130,130,130), Transparency = 1}
        ):Play()
    end)

    -- Shockwave ring (flat cylinder expanding outward)
    local ring = Instance.new("Part")
    ring.Name        = "NukeRing"
    ring.Shape       = Enum.PartType.Cylinder
    ring.Size        = Vector3.new(2, 1, 1)
    ring.CFrame      = CFrame.new(pos + Vector3.new(0,4,0)) * CFrame.Angles(0,0,math.pi/2)
    ring.Anchored    = true
    ring.CanCollide  = false
    ring.CastShadow  = false
    ring.Material    = Enum.Material.Neon
    ring.Color       = Color3.fromRGB(255,120,0)
    ring.Transparency = 0.3
    ring.Parent      = Workspace
    table.insert(parts, ring)
    TweenService:Create(ring,
        TweenInfo.new(1.5, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
        {Size = Vector3.new(2, 140, 140), Transparency = 0.85}
    ):Play()

    -- Mushroom stem (tall column)
    local stem = Instance.new("Part")
    stem.Name        = "NukeStem"
    stem.Size        = Vector3.new(12, 0, 12)
    stem.CFrame      = CFrame.new(pos)
    stem.Anchored    = true
    stem.CanCollide  = false
    stem.CastShadow  = false
    stem.Material    = Enum.Material.Neon
    stem.Color       = Color3.fromRGB(220,100,40)
    stem.Transparency = 0.4
    ...
    TweenService:Create(stem,
        ...
        {Size = Vector3.new(12, 140, 12), CFrame = CFrame.new(pos + Vector3.new(0,70,0))}
    ):Play()
    task.delay(2.5, function()
        TweenService:Create(stem, TweenInfo.new(1.5), {Transparency=1}):Play()
    end)

    -- Mushroom cap (ball on top)
    local cap = Instance.new("Part")
    cap.Name        = "NukeCap"
    cap.Shape       = Enum.PartType.Ball
    cap.Size        = Vector3.new(5,5,5)
    cap.CFrame      = CFrame.new(pos + Vector3.new(0,10,0))
    cap.Anchored    = true
    cap.CanCollide  = false
    cap.CastShadow  = false
    cap.Material    = Enum.Material.Neon
    cap.Color       = Color3.fromRGB(240,140,40)
    ...
    TweenService:Create(cap,
        ...
        {Size = Vector3.new(80,60,80), CFrame = CFrame.new(pos + Vector3.new(0,145,0)), Transparency=0.45}
    ):Play()
    task.delay(2.8, function()
        TweenService:Create(cap, TweenInfo.new(2), {Transparency=1}):Play()
    end)

    -- Small debris parts flying outward
    for _ = 1, 18 do
        local d = Instance.new("Part")
        d.Size        = Vector3.new(1+math.random()*3, 1+math.random()*3, 1+math.random()*3)
        d.CFrame      = CFrame.new(pos + Vector3.new(0,4,0))
        d.Anchored    = false
        d.CanCollide  = false
        d.CastShadow  = false
        d.Material    = Enum.Material.SmoothPlastic
        d.Color       = Color3.fromRGB(180+math.random(60), 60+math.random(60), 0)
        d.Transparency = 0.25
        d.Parent      = Workspace
        table.insert(parts, d)
        local bv = Instance.new("BodyVelocity")
        local dir = Vector3.new(math.random()-0.5, 0.5+math.random(), math.random()-0.5).Unit
        bv.Velocity = dir * (40 + math.random()*60)
        bv.MaxForce = Vector3.new(1,1,1) * 9e8
        bv.P        = 9e8
        bv.Parent   = d
        game:GetService("Debris"):AddItem(bv, 0.5)
        game:GetService("Debris"):AddItem(d, 8)
    end
    ...
    scorch.Size       = Vector3.new(0.2,1,1)
    scorch.CFrame     = CFrame.new(pos + Vector3.new(0,0.12,0)) * CFrame.Angles(0,0,math.pi/2)
    ...
    scorch.Transparency = 0.1
    scorch.Parent     = Workspace
    table.insert(parts, scorch)
    TweenService:Create(scorch,
        TweenInfo.new(1.5),
        {Size = Vector3.new(0.2, 110, 110)}
    ):Play()

    -- Auto-cleanup all explosion parts
...
local function NukeScreenShake(duration, intensity)
    local t = 0
    local shakeConn
    shakeConn = RunService.RenderStepped:Connect(function(dt)
        ...
        local i = intensity * (1 - t/duration)
...
local function TriggerBlueSkyEvent()
    ...
    BlueSky.disgLaunchCount = 0

    local disgraced   = IsDisgraced("Blue Sky")
    local circleRadius = disgraced and 80 or 50
    local countdown    = disgraced and 5 or 10
    local circleColor  = disgraced and Color3.fromRGB(140,0,200) or Color3.fromRGB(255,0,0)
    local nukeTarget   = Vector3.new(hrp.Position.X, hrp.Position.Y, hrp.Position.Z)

    -- Inner function: do one launch sequence (called up to 3 times for disgraced)
    local function DoLaunch(target)
        -- Spawn circle
        if BlueSky.circleRef then BlueSky.circleRef:Destroy() end
        local circle = Instance.new("Part")
        circle.Name        = "NukeCircle"
        circle.Shape       = Enum.PartType.Cylinder
        circle.Size        = Vector3.new(0.25, circleRadius*2, circleRadius*2)
        circle.CFrame      = CFrame.new(target + Vector3.new(0,0.14,0)) * CFrame.Angles(0,0,math.pi/2)
        circle.Anchored    = true; circle.CanCollide = false; circle.CastShadow = false
        circle.Color       = circleColor; circle.Material = Enum.Material.Neon
        circle.Transparency = 0.72; circle.Parent = Workspace
        TweenService:Create(circle, TweenInfo.new(0.6, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut, -1, true),
            {Transparency=0.85}):Play()
        BlueSky.circleRef = circle
        BSShowText("incoming.", 99)

        -- Flash sequence
        local flashRunning  = true
        local flashInterval = disgraced and 1.5 or 3
        local totalFlashTime = 0
        local MAX_FT = countdown

        local function RunFlash()
            if not flashRunning then return end
            TweenService:Create(BSFlash, TweenInfo.new(0.06), {BackgroundTransparency=0.55}):Play()
            task.delay(0.08, function()
                TweenService:Create(BSFlash, TweenInfo.new(0.12), {BackgroundTransparency=1}):Play()
            end)
            totalFlashTime  = totalFlashTime + flashInterval
            flashInterval   = math.max(flashInterval * 0.5, 0.05)
            if totalFlashTime < MAX_FT and flashRunning then
        ...
        task.delay(countdown, function()
            ...
            -- Build nuke
            local nukeSpawnPos = target + Vector3.new(0,150,0)
            local model        = Instance.new("Model"); model.Name = "NukeMissile"
            local nukeScale    = disgraced and 1.6 or 1.0
            local nukeCol      = disgraced and Color3.fromRGB(140,0,220) or Color3.fromRGB(180,180,255)

            local function mkNP(nm, sz, pos2, col, tr)
                local p = Instance.new("Part"); p.Name=nm; p.Size=sz*nukeScale
                p.CFrame=CFrame.new(nukeSpawnPos+pos2); p.Anchored=true; p.CanCollide=false
                p.CastShadow=false; p.Material=Enum.Material.Neon; p.Color=col; p.Transparency=tr
                p.Parent=model; return p
            end
            local body = mkNP("Body", Vector3.new(3,18,3), Vector3.new(0,0,0), nukeCol, 0.55)
            mkNP("Nose", Vector3.new(3,5,3), Vector3.new(0,11.5,0), disgraced and Color3.fromRGB(200,0,255) or Color3.fromRGB(255,80,80), 0.5)
            for i=0,3 do
                local a=math.pi/2*i
                mkNP("Fin"..i, Vector3.new(0.4,6,3), Vector3.new(math.sin(a)*2.2,-6,math.cos(a)*2.2), disgraced and Color3.fromRGB(120,0,180) or Color3.fromRGB(140,140,255), 0.55)
            end
            mkNP("Exhaust", Vector3.new(2,1,2), Vector3.new(0,-9.5,0), Color3.fromRGB(255,160,0), 0.3)
            model.PrimaryPart = body; model.Parent = Workspace
            BlueSky.nukeModel = model

            BSFlash.BackgroundColor3 = Color3.fromRGB(255,255,255)
            TweenService:Create(BSFlash, TweenInfo.new(0.1), {BackgroundTransparency=0.4}):Play()
            task.delay(0.2, function()
                BSFlash.BackgroundColor3 = Color3.fromRGB(255,0,0)
                TweenService:Create(BSFlash, TweenInfo.new(0.4), {BackgroundTransparency=1}):Play()
            end)
            ...
            task.delay(0.1, function()
                ...
                for _, p in ipairs(model:GetDescendants()) do
                    if p:IsA("BasePart") then
                        TweenService:Create(p, TweenInfo.new(2.5, Enum.EasingStyle.Quad, Enum.EasingDirection.In),
                            {CFrame = p.CFrame + Vector3.new(0,-150,0)}):Play()
                    end
                end
            end)
            ...
            task.delay(2.6, function()
                if not BlueSky.active then return end
                if BlueSky.nukeModel then BlueSky.nukeModel:Destroy(); BlueSky.nukeModel = nil end
                if BlueSky.circleRef then BlueSky.circleRef:Destroy(); BlueSky.circleRef = nil end

                SpawnNukeExplosion(target)

                BSFlash.BackgroundColor3 = Color3.fromRGB(255,255,220)
                TweenService:Create(BSFlash, TweenInfo.new(0.05), {BackgroundTransparency=0}):Play()
                task.delay(0.15, function()
                    BSFlash.BackgroundColor3 = Color3.fromRGB(255,80,0)
                    TweenService:Create(BSFlash, TweenInfo.new(0.8), {BackgroundTransparency=1}):Play()
                end)
                NukeScreenShake(4, disgraced and 2.5 or 1.8)

                local hrpNow = HumanoidRootPart
                if hrpNow and not BlueSky.dmgDealt then
                    local dist = (hrpNow.Position - target).Magnitude
                    if dist <= circleRadius then
                        BlueSky.dmgDealt = true
                        TagDeathCause("BlueSky")
                        InstantFateDamage(100)
                        BSShowText("you were in the zone.", 3)
                    elseif disgraced then
                        -- Escaped the circle — launch again if under max
                        BlueSky.disgLaunchCount = BlueSky.disgLaunchCount + 1
                        if BlueSky.disgLaunchCount < BlueSky.MAX_LAUNCHES then
                            task.delay(1.5, function()
                                if not BlueSky.active then return end
                                local hrp2 = HumanoidRootPart
                                if hrp2 then
                                    local newTarget = hrp2.Position
                                    DoLaunch(newTarget)
                                end
                            end)
                            return  -- don't reset inEvent yet
                        end
                    end
                end

                task.delay(5, function() BlueSky.inEvent = false end)
            end)
        end)
    end
...
local function OnBlueSkyEnable()
    ...
    BlueSky.conn = RunService.Heartbeat:Connect(function(dt)
        ...
        if BlueSky.cycleTimer <= 0 then
            BlueSky.cycleTimer = BlueSky.CYCLE_INT
            task.spawn(TriggerBlueSkyEvent)
        end
    end)
end
...
local function OnBlueSkyDisable()
    ...
    if BlueSky.circleRef  then BlueSky.circleRef:Destroy();  BlueSky.circleRef  = nil end
    if BlueSky.nukeModel  then BlueSky.nukeModel:Destroy();   BlueSky.nukeModel  = nil end
    TweenService:Create(BSFlash, TweenInfo.new(0.3), {BackgroundTransparency=1}):Play()
    TweenService:Create(BSText,  TweenInfo.new(0.3), {TextTransparency=1}):Play()
end

RegisterEntity("Blue Sky","Treason",
    "I leaked all of the government info. Nobody can stop me. I could even launch the nuke.",
    ...
    for _, p in ipairs(corpse:GetDescendants()) do
        pcall(function()
            if p:IsA("BasePart") then
                p.Anchored    = true
                p.CanCollide  = false
                p.Color       = Color3.fromRGB(0,0,0)  -- charred black
            end
        end)
    end

    -- Drop corpse to ground
    local groundRay = Workspace:Raycast(
        deathPos + Vector3.new(0,5,0), Vector3.new(0,-30,0), RaycastParams.new()
    )
    local groundY = groundRay and groundRay.Position.Y or deathPos.Y

    -- Lay flat and stretch every limb to extremes
    local stretchOffsets = {
        Torso    = {pos=Vector3.new(0,0,0),      size=Vector3.new(2,2,1)},
        Head     = {pos=Vector3.new(0,1.5,0),    size=Vector3.new(1,1,1)},
        LeftArm  = {pos=Vector3.new(-8,0,0),     size=Vector3.new(12,1,1)},
        RightArm = {pos=Vector3.new(8,0,0),      size=Vector3.new(12,1,1)},
        LeftLeg  = {pos=Vector3.new(-2,-9,0),    size=Vector3.new(1,14,1)},
        RightLeg = {pos=Vector3.new(2,-9,0),     size=Vector3.new(1,14,1)},
    }
    -- Random extra stretch factor per corpse
    local sx = 1 + math.random()*1.5
    local sy = 1 + math.random()*1.5

    local baseRotY = math.random() * math.pi * 2
    local baseCF   = CFrame.new(deathPos.X, groundY + 0.12, deathPos.Z)
                   * CFrame.Angles(math.pi/2, baseRotY, 0)  -- lay flat

    for nm, info in pairs(stretchOffsets) do
        local p = corpse:FindFirstChild(nm)
        if p and p:IsA("BasePart") then
            local targetCF   = baseCF * CFrame.new(info.pos * Vector3.new(sx, sy, 1))
            ...
            TweenService:Create(p,
                ...
                {CFrame = targetCF, Size = targetSize}
            ):Play()
        end
    end

    -- Black smoke particles from the corpse torso
    local torso = corpse:FindFirstChild("Torso")
    if torso then
        task.delay(0.5, function()
            if not torso or not torso.Parent then return end
            -- Simulate smoke with rising black parts
            local smokeRunning = true
            task.spawn(function()
                while smokeRunning do
                    task.wait(0.18)
                    if not torso or not torso.Parent then break end
                    local s = Instance.new("Part")
                    s.Size        = Vector3.new(0.7+math.random()*1.2, 0.7+math.random()*1.2, 0.7+math.random()*1.2)
                    s.CFrame      = CFrame.new(torso.Position + Vector3.new((math.random()-0.5)*3, 0, (math.random()-0.5)*3))
                    s.Anchored    = false
                    s.CanCollide  = false
                    s.CastShadow  = false
                    ...
                    s.Parent      = Workspace
                    local bv = Instance.new("BodyVelocity")
                    bv.Velocity = Vector3.new((math.random()-0.5)*2, 4+math.random()*3, (math.random()-0.5)*2)
                    bv.MaxForce = Vector3.new(1,1,1)*9e8
                    bv.P        = 500
                    bv.Parent   = s
                    -- Fade out
                    TweenService:Create(s,
                        TweenInfo.new(2+math.random()*1.5),
                        {Transparency=1, Size=s.Size*2.5}
                    ):Play()
                    game:GetService("Debris"):AddItem(s, 3.5)
                end
            end)
            task.delay(25, function() smokeRunning = false end)
        end)
    end
